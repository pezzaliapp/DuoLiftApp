// DuoLiftApp 3D — using global THREE (script name `app3d.js`)
(function(){
  const errBox = document.getElementById('err');
  function err(msg){ errBox.textContent = msg; errBox.style.display='block'; console.error(msg); }

  if (!window.THREE || !THREE.OrbitControls){ err('CDN bloccato: impossibile caricare Three.js.'); return; }
  ...
  let dragging=null; let dragOffset=new THREE.Vector3();
  ...
  renderer.domElement.addEventListener('pointerup', ()=>{ if(dragging){ trySnap(dragging); } dragging=null; controls.enabled=true; });
