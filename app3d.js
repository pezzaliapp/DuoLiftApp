// DuoLiftApp 3D — Full working drag & snap puzzle
...
const M = {
  ...
  ghost: new THREE.MeshStandardMaterial({color:0xffffff, opacity:0.15, transparent:true})
};

function box(w,h,d,mat){ const m = new THREE.Mesh(new THREE.BoxGeometry(w,h,d), mat); m.userData.size=[w,h,d]; return m; }
function setPos(o,[x,y,z]){ o.position.set(x,y,z); return o; }
...
function buildTargets(){
  ...
  for (const g of t){
    ...
    setPos(ghost, g.pos);
    ghost.rotation.y = g.rotY;
    ghost.name = 'ghost:'+g.id;
    scene.add(ghost);
  }
  state.targets = t;
}
...
function buildPieces(){
  ...
  for (const g of state.targets){
    const mesh = box(...g.size, g.mat);
    mesh.name = g.id;
    mesh.position.set(rnd(-2.5,2.5), rnd(0.2,1.2), startZ + rnd(-1.2,1.2));
    mesh.rotation.y = 0;
    mesh.userData = {id:g.id, placed:false, target:g};
    scene.add(mesh);
...
renderer.domElement.addEventListener('pointermove', (e)=>{
  ...
  if (raycaster.ray.intersectPlane(dragPlane, hitPoint)){
    dragging.position.copy(hitPoint.sub(dragOffset));
  }
});
...
// Rotate selected piece with Q/E
...
function trySnap(mesh){
  ...
  const angOk = Math.abs(((mesh.rotation.y - t.rotY + Math.PI)%(2*Math.PI))-Math.PI) < (Math.PI/18); // ±10°
  if (posOk && angOk){
...
function checkWin(){
  if (state.pieces.every(p=>p.userData.placed)){
    // Symmetry + arm order
    const ok = validateSymmetry();
    if (ok){ state.score += 250; scoreEl.textContent = String(state.score); toast('✅ Ponte completato!'); }
    else { toast('⚠️ Completato, ma controlla simmetria/ordine bracci.'); }
  }
}
...
function validateSymmetry(){
  ...
  const sym = Math.abs((colL.pos[0]) - (colR.pos[0])) > 1.5 ? false : true;
  const L = [state.targets.find(t=>t.id==='armL_1').size[0], state.targets.find(t=>t.id==='armL_2').size[0], state.targets.find(t=>t.id==='armL_3').size[0]];
  const R = [state.targets.find(t=>t.id==='armR_1').size[0], state.targets.find(t=>t.id==='armR_2').size[0], state.targets.find(t=>t.id==='armR_3').size[0]];
  const order = (L[0]>L[1] && L[1]>L[2]) && (R[0]>R[1] && R[1]>R[2]);
  return sym && order;
}
...
function tick(t){
  if (state.started){
    const sec = Math.floor((t - state.t0)/1000);
...
function toast(msg){
  ...
  el.style.background='#0f5132'; el.style.color='#fff'; el.style.padding='10px 14px'; el.style.borderRadius='10px';
  el.style.boxShadow='0 8px 30px rgba(0,0,0,.35)'; el.style.zIndex='20';
...
function boot(){
  buildTargets();
  // ghosts already in scene; pieces later
}
boot();
...
// Start/Reset
function start(){
  if (!state.started){ state.started = true; state.t0 = performance.now(); intro.style.display='none'; }
  reset();
}
function reset(){
  for (const m of state.pieces){ scene.remove(m); }
  state.pieces = [];
  state.score = 0; scoreEl.textContent='0'; placedEl.textContent='0';
  buildPieces();
}
